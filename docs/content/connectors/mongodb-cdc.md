...
Download [flink-sql-connector-mongodb-cdc-2.3.0.jq.jar](https://repo1.maven.org/maven2/com/ververica/flink-sql-connector-mongodb-cdc/2.3.0.jq/flink-sql-connector-mongodb-cdc-2.3.0.jq.jar) and put it under `<FLINK_HOME>/lib/`.

**Note:** flink-sql-connector-mongodb-cdc-XXX-SNAPSHOT version is the code corresponding to the development branch. Users need to download the source code and compile the corresponding jar. Users should use the released version, such as [flink-sql-connector-mongodb-cdc-2.3.0.jq.jar](https://mvnrepository.com/artifact/com.ververica/flink-sql-connector-mongodb-cdc), the released version will be available in the Maven central warehouse.
...
The MongoDB CDC incremental connector (after 2.3.0.jq) can be used as the following shows:
```java
